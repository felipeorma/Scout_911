...
weights = {
    ...
    # Agrega ponderaciones para otras posiciones aquí...
}
...
def calculate_score(row, position):
    ...
    if position in weights:
        for metric, weight in weights[position].items():
            if metric in row and pd.notna(row[metric]):  # Validación de existencia y no nulo
...
if selected_league:
    try:
        ...
        # Filtrar los datos según la posición seleccionada usando 'Primary position'
        position_map = {
            'Arquero': 'GK',
            'Defensa': 'CB',
        ...
        if min_score != max_score:
            ...
            df['Score Total'] = 5  # Puntaje central si no hay variación en los valores
